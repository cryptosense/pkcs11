...
let pIv = Ctypes_helpers.smart_field t "pIv" (Ctypes_helpers.Reachable_ptr.typ Pkcs11_CK_BYTE.typ)
...
let pAAD = Ctypes_helpers.smart_field t "pAAD" (Ctypes_helpers.Reachable_ptr.typ Pkcs11_CK_BYTE.typ)
...
let make u =
  ...
  Ctypes_helpers.make_string
    (P11_gcm_params.iv u)
    p
    ulIvLen
    pIv;
  Ctypes_helpers.make_string
    (P11_gcm_params.aad u)
    p
    ulAADLen
    pAAD;
