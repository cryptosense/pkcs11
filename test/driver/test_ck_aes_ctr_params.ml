...
let test_make =
  "make" >:: fun ctxt ->
    ...
    let got_block = Ctypes_helpers.string_from_carray (Ctypes.getf low_level Pkcs11_CK_AES_CTR_PARAMS.block) in
...
let test_view =
  let test low_level expected ctxt =
    ...
    assert_equal
      ~ctxt
      ~cmp:[%eq: P11_aes_ctr_params.t]
      ~printer:[%show: P11_aes_ctr_params.t]
      expected
      got
  in
  ...
  [ "Ok" >:: test
      (build
         ~bits
         ~block:(Ctypes_helpers.carray_from_string block)
      )
