...
let assert_equal_object_handle ~ctxt =
  assert_equal ~ctxt ~cmp:[%eq: P11.Object_handle.t]
    ~printer:[%show: P11.Object_handle.t]

let assert_equal_string ~ctxt =
  assert_equal ~ctxt ~cmp:[%eq: string] ~printer:[%show: string]
...
let test_get_info (module R : P11_driver.S) ctxt =
  ...
  let expected =
    { cryptokiVersion = {major = 1; minor = 2}
    ; libraryVersion = {major = 3; minor = 4}
    ; manufacturerID =
        "fake manufacturer ID\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
    ; libraryDescription =
        "fake library description\x00\x00\x00\x00\x00\x00\x00\x00"
    ; flags = Unsigned.ULong.of_int 5 }
  in
  assert_equal ~ctxt ~cmp:[%eq: P11.Info.t] ~printer:[%show: P11.Info.t]
    expected got
...
let test_get_slot_list (module R : P11_driver.S) ctxt =
  ...
  assert_equal ~ctxt ~cmp:[%eq: P11.Slot_id.t list]
    ~printer:[%show: P11.Slot_id.t list] expected_slots got_slots
...
let test_get_attribute_value (module R : P11_driver.S) session key =
  let test attribute_type value ctxt =
    ...
    assert_equal ~ctxt ~cmp:[%eq: P11.Attribute.pack option]
      ~printer:[%show: P11.Attribute.pack option] expected got
  in
  [ "CKA_CLASS"
    >:: test P11.Attribute_type.CKA_CLASS P11.Object_class.CKO_SECRET_KEY
  ...
  ; "CKA_VALUE"
    >:: test P11.Attribute_type.CKA_VALUE
          "\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0a\x0b\x0c\x0d\x0e\x0f"
  ; "CKA_VALUE_LEN"
    >:: test P11.Attribute_type.CKA_VALUE_LEN (Unsigned.ULong.of_int 16)
  ...
  ; "CKA_MODULUS"
    >:: test P11.Attribute_type.CKA_MODULUS
          (P11.Bigint.decode
             "\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0a\x0b\x0c\x0d\x0e\x0f")
  ; "CKA_PUBLIC_EXPONENT"
    >:: test P11.Attribute_type.CKA_PUBLIC_EXPONENT (P11.Bigint.of_int 65537)
  ; "CKA_EC_PARAMS"
    >:: test P11.Attribute_type.CKA_EC_PARAMS
          ""
  ]

let test_set_attribute_value (module R : P11_driver.S) session key _ctxt =
  R.set_attribute_value session key []

let test_generate_key_pair (module R : P11_driver.S) session ctxt =
  let (pub, priv) =
    R.generate_key_pair session P11.Mechanism.CKM_AES_KEY_GEN [] []
  in
...
let test_verify (module R : P11_driver.S) session key =
  ...
  in
  let test_valid ctxt = assert_equal ~ctxt () (verify "OK") in
  let test_invalid _ctxt =
    assert_raises (P11_driver.CKR P11.RV.CKR_SIGNATURE_INVALID) @@ fun () ->
    verify "Invalid"
  in
  ["Valid" >:: test_valid; "Invalid" >:: test_invalid]
...
let open_session (module R : P11_driver.S) =
  let session =
    R.open_session ~slot:Unsigned.ULong.zero
...
let test_object driver =
  ...
  let key =
    R.generate_key session P11.Mechanism.CKM_AES_KEY_GEN
      [ P11.Attribute.Pack
          (P11.Attribute_type.CKA_VALUE_LEN, Unsigned.ULong.of_int 32) ]
  in
