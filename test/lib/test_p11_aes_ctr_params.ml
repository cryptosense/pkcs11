...
let test_make =
  let test_ok ~bits ~block ctxt =
    ...
    let got =
      (P11_aes_ctr_params.bits params, P11_aes_ctr_params.block params)
    in
  ...
  "make"
  >::: [ "Ok"
         >:: test_ok ~bits:(Unsigned.ULong.of_int 16) ~block:"AAAABBBBCCCCDDDD"
       ]

let test_to_yojson =
  let test params expected ctxt =
    let got = P11_aes_ctr_params.to_yojson params in
    assert_equal ~ctxt ~cmp:[%eq: yojson] ~printer:[%show: yojson] expected got
  in
  "to_yojson"
  >::: [ "normal"
         >:: test
               (P11_aes_ctr_params.make ~bits:(Unsigned.ULong.of_int 64)
...
let test_of_yojson =
  let test json expected ctxt =
    ...
    assert_equal ~ctxt ~cmp:[%eq: (P11_aes_ctr_params.t, string) Result.result]
      ~printer:[%show: (P11_aes_ctr_params.t, string) Result.result] expected
      got
  in
  "of_yojson"
  >::: [ "normal"
...
let suite = "AES_CTR_params" >::: [test_make; test_to_yojson; test_of_yojson]
