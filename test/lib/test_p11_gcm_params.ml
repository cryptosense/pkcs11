...
let test_of_yojson =
  ...
  assert_equal ~ctxt ~cmp:[%eq: (P11_gcm_params.t, string) Result.result]
    ~printer:[%show: (P11_gcm_params.t, string) Result.result] expected got
...
let suite = "GCM_params" >::: [test_make; test_of_yojson; test_to_yojson]
